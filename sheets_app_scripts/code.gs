   ...
   for (var i=0;i<sourceLen;i++){
     if(sourceData[i][1] !== '') {
       var data = {};
        ...
        var timestamp = new Date(sourceData[i][3]);
       ...
       data.tags = JSON.parse(sourceData[i][12]);
   ...
   for (var i=0;i<sourceLen;i++){
     if(sourceData[i][1] !== '') {
      ...
      data.id = sourceData[i][0];
       ...
       data.salePercent = sourceData[i][2];
       data.endDate = timestamp;
       data.backgroundImage = sourceData[i][4];
      
       firestore.createDocument("promotions/"+data.id,data);
...
function fetchTags() {
  ...
  // Loop through the rows
   for (var i=0;i<sourceLen;i++){
     if(sourceData[i][0] !== '') {
       var data = {};
        data.id = sourceData[i][1];
      data.name = sourceData[i][0];
      
       firestore.createDocument("tags/"+data.id,data);
